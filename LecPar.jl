...
@everywhere function LecturaJsonPar()
    ...
    #Declaraciones Globales: Tiempo de Simulación, Número de Iteraciones
    ...
    numberEquip=0
    numberEvents=0
    dataEv=0                                # Inicialización contador de equipos
    dataEq=0
    MaxEv=0
    va1=0

    #Bucle para contar la cantidad de equipos y eventos de cada sistema
    for i=1:(Nsis-1)      # Nsis-1 cuando hay más de un sistema
        ...
        if va1[1]=='{'                  # Evalúa si el sistema tiene un solo equipo para corregir bug provocado en el arhivo JSON
            va1=string("[", va1,"]")
            dataEq=JSON.parse(va1)                  # Corrección bug
        else
    ...
    for i=1:(Nsis-1)        # Se recorre hasta el número de sistemas
        ...
        for j=1:length(dataEq)      # Se recorre hasta el número de equipos de cada sistema
            ...
            for k=1:length(dataEv)      # Se recorre hasta el número de eventos de cada equipo de cada sistema
                #Evaluación del tipo de evento, puesto que si es un Mantenimiento, la ocurrencia del evento es constantte.
    ...
    acu=1;
    cont=1;
    global idsEv=Array{Any}(numberEvents,3)    # Reinicio de Acumulador
    #Variable que contendrá los valores de los timeos máximo, mínimo y de paso del manteniento por cada sistema.
    #Bucle de Llenado de los vectores Durations y Costs
    for i=1:(Nsis-1) #se recorre hasta el número de sistemas
        va1=JSON.json(data["lPLModeler:Process"]["systems"][i]["equipments"])       #Corrección del "Bug#1"
        if va1[1]=='{'
            va1=string("[", va1,"]")
            dataEq=JSON.parse(va1)
        else
        ...
        for j=1:length(dataEq)     #se recorre hasta el número de equipos de cada sistema
            ...
            for k=1:length(dataEv)  # se recorre hasta el número de eventos de cada equipo de cada sistema
                #Si el evento es de tpo Falla, se evalúan los tipos de distibuciones de probabilidad de la duración y se asignan los valores según corresponda.
                ...
                acu =acu+4; #aumento del acumulador para asignar los parámetros de un nuevo evento.
    ...
    if length(data["lPLModeler:Process"]["systems"][1])==9||1==1
        ...
        global Ngr=length(dataGr)                   # Cantidad de grupos por sistema
